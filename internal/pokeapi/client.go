...
func GetLocationEncounters(location string) (*LocationEncounters, error) {
	url := "https://pokeapi.co/api/v2/location-area/" + location
	cachedValue, ok := globalCache.Get(location)
	var data []byte
	if ok {
		data = cachedValue
	} else {
		...
		globalCache.Add(location, data)

	}
	var locationEncounters LocationEncounters
	err := json.Unmarshal(data, &locationEncounters)
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal JSON: %w", err)
	}

	if len(locationEncounters.PokemonEncounters) <= 0 {
		return nil, fmt.Errorf("no Pokemon in the location")
	}
...
func GetPokemon(name string) (*Pokemon, error) {
	...
	cachedValue, ok := globalCache.Get(name)
	var data []byte
	if ok {
		data = cachedValue
	} else {
